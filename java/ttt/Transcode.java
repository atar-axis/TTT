...
public class Transcode implements ControllerListener, DataSinkListener {
     ...
     */
    public boolean doIt(MediaLocator inML, MediaLocator outML, Format fmts[], int start, int end, float jpegQuality) {
        ...
        if (start > 0)
            p.setMediaTime(new Time((long) 1000000l * start));

        // Set the stop time if there's one set.
        if (end > 0)
            p.setStopTime(new Time((long) 1000000l * end));
    ...
    void setContentDescriptor(Processor p, MediaLocator outML) {
        ...
        if ((cd = fileExtToCD(outML.getRemainder())) != null) {
            ...
            if (cd.toString().endsWith("RTP")) {
                loop = true;
                System.err.println("- repeat playing in a loop");
            }
    ...
    boolean setTrackFormats(Processor p, Format fmts[]) {
        ...
        // System.out.println("\nTrack "+i+": (supported formats)");
        // Format[] supported = tcs[i].getSupportedFormats();
        // for (int j = 0; j < supported.length; j++) {
        // System.out.println(" Format: "+j+": "+supported[j]);
        // }
        // System.out.println();
        // }
        // System.exit(0);
     ...
     */
    void setJPEGQuality(Processor p, float val) {
    ...
    public void controllerUpdate(ControllerEvent evt) {
        ...
        } else if (evt instanceof MediaTimeSetEvent) {} else if (evt instanceof StopAtTimeEvent) {
    ...
    ContentDescriptor fileExtToCD(String name) {
        ...
        if (ext.equals("mp3") || ext.equals("mp2")) {
    ...
    public static void main(String[] args) {
        ...
        while (i < args.length) {
            ...
            if (args[i].equals("-v")) {
                ...
                if (i >= args.length)
                    prUsage();
                Float quality = Float.valueOf(args[i]);
                if (quality != null)
                    jpegQuality = quality.floatValue();
                System.out.println("Quality: " + jpegQuality);
            } else {
                inputURL = args[i];
            }
            i++;
        ...
        for (i = 0; i < audFmt.size(); i++) {
            ...
            if ((fmt = parseAudioFormat((String) audFmt.elementAt(i))) == null) {
                System.err.println("Invalid audio format specification: " + (String) audFmt.elementAt(i));
                prUsage();
            }
            fmts[j++] = fmt;
        }
        ...
        for (i = 0; i < vidFmt.size(); i++) {
            ...
            if ((fmt = parseVideoFormat((String) vidFmt.elementAt(i))) == null) {
                System.err.println("Invalid video format specification: " + (String) vidFmt.elementAt(i));
                prUsage();
            }
            fmts[j++] = fmt;
        }
        ...
        if (!transcode.doIt(iml, oml, fmts, mediaStart, mediaEnd, jpegQuality)) {
    ...
    /**
     * Parse the audio format specifier and generate an AudioFormat. A valid audio format specifier is of the form:
    ...
    /**
     * Parse the video format specifier and generate an VideoFormat. A valid video format specifier is of the form:
    ...
    static void prUsage() {
        System.err
                .println("Usage: java Transcode2 -o <output> -a <audio format> -v <video format> -s <start time> -e <end time> -q <jpeg quality> <input>");
        ...
        System.err.println("     <start time>: start time in milliseconds");
        System.err
                .println("     <jpeg quality>: jpeg quality factor between 0.0 and 1.0 (default 0.5) [only for jpeg formats]");
        System.err.println("     <end time>: end time in milliseconds");
        System.exit(0);
    }
}
