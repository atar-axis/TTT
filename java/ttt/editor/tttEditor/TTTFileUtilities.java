...
public class TTTFileUtilities {
    ...
    // given the name of a file, determines if a file has a valid desktop, audio or video file extension
    // otherwise returns 0
    /**
     * Tests the file suffix of a passed file, and determines whether it is a valid suffix for use with the TTT. If so,
     * an int is returned to indicate which type of file it is - audio, video or desktop. <br />
     * If the file is not valid, 0 is returned.
     * 
     * @param file
     *            the <code>File</code> to check
     * @return the type of file which was passed
     */
    public static int getFileType(File file) {
        String fileName = file.getName();
        for (int count = 0; count < Parameters.desktopEndings.length; count++) {
            if (fileName.endsWith(Parameters.desktopEndings[count]))
                return DESKTOP;
        }
        for (int count = 0; count < Parameters.audioEndings.length; count++) {
            if (fileName.endsWith(Parameters.audioEndings[count]))
                return AUDIO;
        }
        for (int count = 0; count < Parameters.videoEndings.length; count++) {
            if (fileName.endsWith(Parameters.videoEndings[count]))
                return VIDEO;
        }
        return 0;
    }
    ...
    public static void createFileChooser() {
        ...
        String lastUsedPath = TTTEditor.userPrefs.get("last used path", null);
    ...
    public static File showOpenFileInternalDialog(boolean allowOlderDesktopFiles) {
        ...
        int returnValue = fileChooser.showOpenDialog(TTTEditor.getInstance());

        if (returnValue == JFileChooser.APPROVE_OPTION) {
            // added 22.03.2006 by Peter Ziewer
            // use same path after restart
            TTTEditor.userPrefs.put("last used path", fileChooser.getSelectedFile().getAbsoluteFile().getParent());
    ...
    public static File showSaveFileInternalDialog(File suggestedFile) {
        ...
        int returnValue = fileChooser.showSaveDialog(TTTEditor.getInstance());
        if (returnValue == JFileChooser.APPROVE_OPTION)
            return fileChooser.getSelectedFile();
