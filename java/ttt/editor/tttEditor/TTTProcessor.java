...
//        String basicName = TTTFileUtilities.getBasicFileName(outputFile);
     ...
     * @return <code>true</code> if the file trim was successful, <code>false</code> otherwise
     */
    public static boolean trim(FileConnector fileConnector, File outputFile, int startTimeMS, int endTimeMS,
            boolean forSubdivide) {

        if (outputFile == null)
            return false;

        System.out.println("Beginning trim...");
        System.out.println("Start time:\t" + TTTEditor.getStringFromTime(startTimeMS, true));
        System.out.println("End time:\t" + TTTEditor.getStringFromTime(endTimeMS, true));
        System.out.println();
        ...
        File videoFile = fileConnector.getVideoFile();
        if (videoFile != null) {
            ...
            if (videoSuccessful) {
                ...
                System.out.println("start:\t" + TTTEditor.getStringFromTime(cuttedStart, true) + "\t("
                        + TTTEditor.getStringFromTime(cuttedStart - startTimeMS * 1000000l, true) + ")");
                System.out.println("end:\t" + TTTEditor.getStringFromTime(cuttedEnd, true) + "\t("
                        + TTTEditor.getStringFromTime(cuttedEnd - endTimeMS * 1000000l, true) + ")");
                System.out.println("diff:\t" + TTTEditor.getStringFromTime(cuttedEnd - cuttedStart, true));
                System.out.println();
            } else {
        ...
        File audioFile = fileConnector.getAudioFile();
        if (audioFile != null) {
            ...
            // audioSuccessful = cutAudio(audioFile, audioOutput, startTimeMS, endTimeMS);
        }
        ...
        try {
            ...
            // use real video cutting timestamps (better sync)
            if (forSubdivide)
                desktopSuccessful = trimDesktopForSubdivide(fileData, outputFile, cuttedStart, cuttedEnd);
            // desktopSuccessful = trimDesktopForSubdivide(fileData, outputFile, startTimeMS, endTimeMS);
            else
                desktopSuccessful = trimDesktop(fileData, outputFile, cuttedStart, cuttedEnd);
            // desktopSuccessful = trimDesktop(fileData, outputFile, startTimeMS, endTimeMS);
        } catch (Exception e) {
    ...
    public static File[] getFilesForConcat() {
        ...
        int selection = JOptionPane.showInternalOptionDialog(TTTEditor.getInstance().getDesktopPane(), selector,
    ...
    private static boolean concatDesktop(File[] desktopFiles, File outputFile, long[] mediaDurations) throws Exception {
        ...
        TTTFileData[] desktopData = new TTTFileData[desktopFiles.length];
        for (int i = 0; i < desktopFiles.length; i++) {
            try {
                desktopData[i] = TTTFileReader.readFile(desktopFiles[i]);
            } catch (Exception e) {
