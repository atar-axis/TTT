...
/*
 * Edited for use with TTTEditor.
 * Changes allow the durations of the media to be obtained before
...
public class Concat implements ControllerListener, DataSinkListener {
    ...
    MediaLocator inML[], outML;

    /**
     * Given an array of input media locators and an output locator,
     * this method will create the processors which may later be used for the concat operation.
     */
    public boolean createProcessors(MediaLocator inML[], MediaLocator outML) {
        this.inML = inML;
        this.outML = outML;
	...
	pInfo = new ProcInfo[inML.length];
    ...
    /**
     * Gets an array containing the durations of each of the media files to be concatenated.
     * Assumes processors have already been created.
     */
    public long [] getDurationNanoSeconds() {
        if (pInfo == null)
            return null;
        
        long [] durations = new long[pInfo.length];
        for (int i = 0; i < pInfo.length; i++) {
            durations[i] = pInfo[i].p.getDuration().getNanoseconds();
        }
        return durations;
    }
