...
public class Cut implements ControllerListener, DataSinkListener {
    ...
    // store real cutting time (which slightly defer from set times) in order to sync audio and video cutting
    public long realStart = -1;
    public long realEnd = -1;
    ...
    /**
     * Transcode the MPEG audio to linear and video to JPEG so we can do the cutting.
     */
    ...
    /**
     * Block until the given processor has transitioned to the given state. Return false if the transition failed.
     */
    ...
    ContentDescriptor fileExtToCD(String name) {
        ...
        /* Added for TTTEditor */
        if (ext.startsWith("bak"))
            return fileExtToCD(name.substring(0, p - 1));
        // ////////////////////////////////
...
//                    if ((buffer.getFormat() instanceof VideoFormat))
        //                if (((start[0] - 1000000000 < buffer.getTimeStamp()) && ((buffer.getTimeStamp() < start[0] + 1000000000)))
        //                        || ((end[0] - 1000000000 < buffer.getTimeStamp()) && (buffer.getTimeStamp() < end[0] + 1000000000))) {
        //                    System.out.print(buffer.getTimeStamp() + "\t"
        //                            + TTTEditor.getStringFromTime((int) (buffer.getTimeStamp() / 1000000), true) + "\t"
        //                            + buffer.getSequenceNumber() + "\t" + buffer.getOffset() + "\t" + buffer.getLength() + "\t"
        //                            + buffer.getDuration());
        //                    if (buffer.getFormat() instanceof VideoFormat) {
        //                        // The starting frame needs to be a key frame.
        //                        if ((buffer.getFlags() & Buffer.FLAG_KEY_FRAME) != 0) {
        ...
        //                }
                    ...
                    // add by Ziewer 30.03.2006
                    // store real cutting time (which slightly defer from set times) in order to sync audio and video cutting
                    if (buffer.getFormat() instanceof VideoFormat) {
                        if (realStart == -1) {
                            realStart = buffer.getTimeStamp();
                            // System.out.println("real start " + TTTEditor.getStringFromTime((int) (realStart / 1000000l),
                            // true));
                        }
                        if (realEnd <= 0) {
                            if (buffer.getLength() == 0) {
                                // end reached - set previous timestamp
                                // NOTE: add duration of last buffer (= one frame)
                                // System.out.println("\nfirst empty buffer\t"
                                // + TTTEditor.getStringFromTime((int) (buffer.getTimeStamp() / 1000000l), true));
                                // System.out.println("real end\t"
                                // + TTTEditor.getStringFromTime((int) (-realEnd / 1000000l), true));
                                realEnd = -realEnd + (long) (1000000000l / ((VideoFormat) buffer.getFormat()).getFrameRate());
                                //
                                // System.out.println("adjusted end\t"
                                // + TTTEditor.getStringFromTime((int) (realEnd / 1000000l), true));
                                // System.out.println();
                            } else
                                // keep timestamp (negated to mark unfinshed status)
                                realEnd = -buffer.getTimeStamp();
                        }
                    }
        ...
        /**
         * Given a buffer, check to see if this should be included or skipped based on the start and end times.
         */
        ...
        boolean checkStartTime(Buffer buf, long startTS) {
            if (isRawAudio(buf.getFormat())) {
                ...
                // added by Ziewer 29.03.2006
                // this is used for mp3
            } else if (buf.getFormat() instanceof AudioFormat) {
                // mp3 cutting fix
                // NOTE: timestamps of mp3-buffer are somehow strange:
                // seq Nr.1 has timestamp 0ms and data 0-1 sec
                // seq Nr.2 has also timestamp 0ms, but data 1-2 sec
                // seq Nr.3 has timestamp 1s and data 2-3 sec
                // seq Nr.4 has timestamp 2s and data 3-4 sec
                // seq Nr. before last seq Nr.has timestamp of last sec (rounded to lower sec) -1 and data of last sec
                // (may be less than one sec)
                // last seq Nr. has timestamp of last sec and no data

                // check if buffer contains start time
                if (((buf.getSequenceNumber()) * 1000) > (startTS / 1000000)) {
                    // cut first buffer to match start time
                    int offset = (int) ((startTS / 1000000 - (buf.getSequenceNumber() - 1) * 1000) * (((AudioFormat) buf
                            .getFormat()).getFrameRate() / 1000));
                    offset = Math.min(buf.getLength(), offset);

                    buf.setOffset(offset);
                    buf.setLength(buf.getLength() - offset);
        ...
        boolean checkEndTime(Buffer buf, long endTS) {
            ...
            // this is used for mp3
                ...
                // mp3 cutting fix (only works if buffer contains one second of mp3 audio)
                // NOTE: timestamps of mp3-buffer are somehow strange:
                // seq Nr.1 has timestamp 0ms and data 0-1 sec
                // seq Nr.2 has also timestamp 0ms, but data 1-2 sec
                // seq Nr.3 has timestamp 1s and data 2-3 sec
                ...
                // check if buffer contains end time
                if (((buf.getSequenceNumber()) * 1000) > (endTS / 1000000)) {
                    // cut last buffer to match end time
                    int len = (int) ((endTS / 1000000 - (buf.getSequenceNumber() - 1) * 1000) * (((AudioFormat) buf
                            .getFormat()).getFrameRate() / 1000));
                    len = Math.min(buf.getLength(), len);
                    buf.setLength(len);

                    // next puffer can be drown away
                    drop_buffer = true;
                    // but this (cutted) buffer is still needed
                    return false;
                }
            }
            // added by Ziewer 29.03.2006
            // NOTE: get frame which ENDS before endtime
            else if (buf.getFormat() instanceof VideoFormat) {
                long frameDuration = (long) (1000000000 / ((VideoFormat) buffer.getFormat()).getFrameRate());

                if (buf.getTimeStamp() + frameDuration >= endTS) {
                    return true;
                }
        ...
        public int computeLength(long duration, Format fmt) {
            ...
            return (int) ((((duration / 1000) * (af.getChannels() * af.getSampleSizeInBits())) / 1000)
