...
public class JMFUtils {
    ...
    public static MediaPlayer createMediaPlayer(String nameUrl, Frame frame, String audioEffect, String videoEffect) {
            ...
            try {
                ...
                if (sh.configure()) {
                    ...
                    for (int i = 0; i < tc.length; i++) {
                        if (audioEffect != null && tc[i].isEnabled() && tc[i].getFormat() instanceof AudioFormat) {
                            assignEffect(tc[i], audioEffect);
                        } else if (videoEffect != null && tc[i].isEnabled() && tc[i].getFormat() instanceof VideoFormat) {
        ...
        if (mediaPlayer.getPlayer() == null) {
            MessageDialog.createErrorDialog(frame, JMFI18N.getResource("jmstudio.error.player.createfor") + " "
                    + nameUrl);
            return (null);
        }
    ...
    // Handles cases such as merging the two datasources, creating only one
    // datasource if both device names are the same - that is, the device handles
    // both audio and video capture
    public static DataSource createCaptureDataSource(String strAudioDevice, Format audioFormat, String strVideoDevice,
            Format videoFormat) {
    ...
    // added by Peter Ziewer
    // Creates capture DataSources for the requested device names and formats
    // Handles cases such as merging the two datasources, creating only one
    // datasource if both device names are the same - that is, the device handles
    // both audio and video capture
    public static DataSource createCaptureDataSource(MediaLocator strAudioDevice, Format audioFormat,
            MediaLocator strVideoDevice, Format videoFormat) {
    ...
    public static DataSource initializeCaptureDataSource(DataSource ds, String deviceName, Format format) {
        ...
        if (ds == null) {
            ...
            // // patched by Peter Ziewer:
            // // CaptureDeviceManager delivers MediaLocatore with a sample rate of 44100Hz,
            // // but old Linux kernel sound driver supports sample rates up to 22050Hz only
            // if(deviceURL.toString().matches("javasound://44100") && format
            // instanceof AudioFormat){
            // AudioFormat audioFormat = (AudioFormat) format;
            // if(audioFormat.getSampleRate()<44100)
            // deviceURL=new MediaLocator("javasound://22050");
            // }
            // // end of patch
    ...
    // FormatControl exposed by the CaptureDevice (data source)
    // Uses the specified data source ds or creates a new one if null
    public static DataSource initializeCaptureDataSource(DataSource ds, MediaLocator deviceName, Format format) {
        MediaLocator deviceURL;
        DataSource dataSource = ds;
        FormatControl formatControls[];
        Format formats[];
        ...
        if (ds == null) {
            ...
            // // patched by Peter Ziewer:
            // // CaptureDeviceManager delivers MediaLocatore with a sample rate
            // of 44100Hz,
            // // but old Linux kernel sound driver supports sample rates up to
            // 22050Hz only
            // if(deviceURL.toString().matches("javasound://44100") && format
            // instanceof AudioFormat){
            // AudioFormat audioFormat = (AudioFormat) format;
        ...
        for (int i = 0; i < formatControls.length; i++) {
            ...
            Format matchingFormat = matches(format, formats);
            if (matches(matchingFormat, formats) != null) {
                formatControls[i].setFormat(matchingFormat);
                return dataSource;
            }
        }
        return null;
    ...
    static Format matches(Format format, Format supported[]) {
        if (supported == null)
            return null;
    ...
    public static RTPSessionMgr createSessionManager(String strAddress, String strPort, String strTtl,
    ...
    public static RTPSessionMgr createSessionManager(String strAddress, int nPort, int nTtl,
            ReceiveStreamListener listener) {
        RTPSessionMgr mngrSession;
