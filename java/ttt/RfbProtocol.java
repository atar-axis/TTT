...
public class RfbProtocol extends GraphicsContext implements Runnable,
	...
	public void requestFullscreenUpdate() throws IOException {
		...
		Constants.writeFramebufferUpdateRequestMessage(
				connection.getOutputStream(), 0, 0, prefs.framebufferWidth,
	...
	public void processProtocol() throws Exception {
		Constants.writeFramebufferUpdateRequestMessage(os, 0, 0,
				prefs.framebufferWidth, prefs.framebufferHeight, false);
		...
		while (running) {
			...
			case Constants.FramebufferUpdate:
				...
				for (int i = 0; i < updateNRects; i++) {
					...
					// System.out.println(rx + "," + ry + " - " + rw + "," + rh
					// + " Encoding "
					...
					// if (updateRectEncoding == Constants.EncodingNewFBSize ) {
					...
					case Constants.EncodingRaw:
						handleMessage(new RawMessage(timestamp, rx, ry, rw, rh,
								this, is));
						break;
					case Constants.EncodingCopyRect:
						handleMessage(new CopyRectMessage(timestamp, rx, ry,
								rw, rh, this, is));
						break;
					case Constants.EncodingHextile:
						// if (!raw)
						handleMessage(new HextileMessage(timestamp, rx, ry, rw,
								rh, this, is));
						// else {
						// System.out.println("HEXTILE -> INTERLACED");
						// new HextileMessage(timestamp, rx, ry, rw, rh, this,
						// is);
						// handleMessage(InterlacedRawMessage
						// .createInterlacedMessages(this, timestamp, rx, ry,
						// rw, rh));
						// }
						break;
					case Constants.EncodingXCursor:
					case Constants.EncodingRichCursor:
						handleMessage(new CursorMessage(updateRectEncoding,
								timestamp, rx, ry, rw, rh, this, is));
						break;
					case Constants.EncodingPointerPos:
						handleMessage(new CursorPositionMessage(timestamp, rx,
								ry, this));
						break;
					default:
			...
			case Constants.SetColourMapEntries:
				// throw new
				// Exception("Can't handle SetColourMapEntries message");
				// ignore
				System.out
						.println("skip unsupported SetColourMapEntries message");
				is.skipBytes(3);
	...
	private int updateRectX, updateRectY, updateRectW, updateRectH,
			updateRectEncoding;
	...
	private void readFramebufferUpdateRectHdr() throws Exception {
		...
		if (updateRectEncoding == Constants.EncodingLastRect
				|| updateRectEncoding == Constants.EncodingNewFBSize)
			return;

		if (updateRectX + updateRectW > prefs.framebufferWidth
				|| updateRectY + updateRectH > prefs.framebufferHeight) {
	...
	static {
		...
		Dimension dimension = Toolkit.getDefaultToolkit().getBestCursorSize(4,
				4);
		...
		invisibleCursor = Toolkit.getDefaultToolkit().createCustomCursor(image,
	...
	public void handleMessage(Message message) {
		...
		// handle other
			...
			// keep cursor for late comers
				...
				// NOTE: no need for calling paint, because messages from VNC
				// Server are already painted
			}
			...
			// enable/disable whiteboard
				...
				if (!isWhiteboardEnabled())
					try {
						Constants.writeFramebufferUpdateRequestMessage(os, 0,
			...
			// NOTE: must be done AFTER whiteboard message (which deletes
			// annotations)
