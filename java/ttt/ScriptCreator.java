...
public class ScriptCreator {
    ...
    final static int OCR_OPTIMIZED = 2;
    final static int THUMBNAILS = 4;
    final static int PDF_SCRIPT = 8;
    ...
    void writeOCRScreenshot(int nr, Image screenshot) {
        ImageCreator.writeImage(screenshot, ocr_path + File.separator + file_base + "." + ((nr + 1) < 10 ? "0" : "")
                + ((nr + 1) < 100 ? "0" : "") + (nr + 1) + ".png");
    }

    // create html, screenshot and thumbnail for current index
    void writeIndex(int nr, Image screenshot) {
        // write screenshot
    ...
    static Image getScaledInstance(Image img, int targetWidth, int targetHeight) {
