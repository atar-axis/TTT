...
public class TTT extends JFrame {
    static final String version = "09.10.2009";
    ...
    private JMenuBar createMenu() {
        ...
        // deactivate F10 as key to open first menu, because F10 has other function in Paint Controls
        menuBar.getInputMap(JMenuBar.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_F10, 0, false),
                "none");
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                try {
                    createOnlinePlayer(InetAddress.getByName("teleteaching.uni-trier.de"), 33229, Constants.Multicast);
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                try {
                    createOnlinePlayer(InetAddress.getByName("teleteaching.uni-trier.de"), 33229, Constants.Unicast);
                } catch (UnknownHostException e) {}
            }
        });
        submenu.add(menuItem);
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                try {
                    createOnlinePlayer(InetAddress.getByName("tele1.uni-trier.de"), 33229, Constants.Multicast);
                } catch (UnknownHostException e) {}
            }
        });
        submenu.add(menuItem);
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                try {
                    createOnlinePlayer(InetAddress.getByName("tele1.uni-trier.de"), 33229, Constants.Unicast);
                } catch (UnknownHostException e) {}
            }
        });
        submenu.add(menuItem);
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                fileChooser.setCurrentDirectory(new File(userPrefs.get("record_path", ".")));

                // ask user
                int returnVal = fileChooser.showDialog(getInstance(), "set recording folder");
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    try {
                        File file = fileChooser.getSelectedFile();
                        // userPrefs.put("record_path", file.getCanonicalFile().getParent());
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                if (JOptionPane.YES_OPTION == JOptionPane.showInternalConfirmDialog(desktop,
                        "Delete all Lecture Profiles?", "TTT: Lecture Profiles", JOptionPane.YES_NO_OPTION))
                    ...
                    new Thread(new Runnable() {
                        public void run() {
                            try {
                                ...
                                userPrefs.put("last_opened_recording", file.getCanonicalPath());
                                setVerbose(verbose);
                        ...
                        public void run() {
                            try {
                                ...
                                try {
                                    ...
                                    showMessage("No errors detected in '" + file + "'",
                                            "TTT: Testing XML Searchbase File", JOptionPane.INFORMATION_MESSAGE);

                                } catch (Throwable t) {
                                    t.printStackTrace();
                                    showMessage("XML file '" + file + "' contains errors.\nError: " + t
                                            + "\n\nSee console output for details.",
                                            "TTT: Testing XML Searchbase File", JOptionPane.ERROR_MESSAGE);
                                }
                            } catch (Exception e) {}
                        }
                    }).start();
        ...
        menuItem = new JCheckBoxMenuItem("generate additional flash movie without controls", TTT.userPrefs.getBoolean(
                "write_flash_without_controls", false));
        menuItem.setToolTipText("useful if converting flash movie to other formats");
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                TTT.userPrefs.putBoolean("write_flash_without_controls", ((JCheckBoxMenuItem) e.getSource())
                        .isSelected());
            }
        });
        submenu.add(menuItem);
        ...
        menuItem.setToolTipText("specify the file which contains your private ssh key");
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
        ...
        menuItem.setToolTipText("specify the file which contains all known hosts/computers");
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                JFileChooser chooser = new JFileChooser();
                chooser.setDialogTitle("Choose known hosts file (ex. ~/.ssh/known_hosts)");
                chooser.setFileHidingEnabled(false);
                int returnVal = chooser.showOpenDialog(TTT.getInstance());
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    TTT.userPrefs.put("ssh_known_hosts", chooser.getSelectedFile().getAbsolutePath());
                    System.out.println("Known hosts file set to '" + chooser.getSelectedFile().getAbsolutePath() + "'");
                }
            }
        });
        submenu.add(menuItem);
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                // ask user
                if (JOptionPane.OK_OPTION == JOptionPane.showConfirmDialog(getInstance(),
                        "Continue to delete and reset all user settings", "Reset all options",
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                showMessage(ABOUT, "About TeleTeachingTool", JOptionPane.INFORMATION_MESSAGE);
            }
        });
        ...
        menuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                showMessage(LICENSE, "TeleTeachingTool - GNU General Public License", JOptionPane.INFORMATION_MESSAGE);
            }
        });
    ...
    private void connectTTT() {
        ...
        final JTextField hostField = new JTextField(userPrefs.get("last_used_host", ""), 12);
        final NumberField portField = new NumberField(5);
        try {
            portField.setText("" + Integer.parseInt(userPrefs.get("last_used_port", "")));
        ...
        multicastCheckBox.setSelected(userPrefs.getBoolean("multicast_client", true));
        ...
        ActionListener actionListener = new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                try {
                    ...
                    final int port = portField.getNumber() >= 100 ? portField.getNumber()
                            : (portField.getNumber() + 5900);

                    final InetAddress hostAddress = InetAddress.getByName(hostField.getText());
                    ...
                    // start player
                    createOnlinePlayer(hostAddress, port, multicast ? Constants.Multicast : Constants.Unicast);
    ...
    public static void showMessage(final Object message, final String title, final int messageType) {
        // console output
        ...
        // workaround: InternalDialog used by open must be invoked in event dispatching thread
            ...
            if (SwingUtilities.isEventDispatchThread())
                JOptionPane.showInternalMessageDialog(getInstance().desktop, message, title, messageType);
            else
                try {
    ...
    static final String ABOUT = "TeleTeachingTool   -   Version " + TTT.version + "\n\n"
            ...
            + "   TTT Protocol Version " + Constants.VersionMessageTTT + "   RFB Protocol Version "
            + Constants.VersionMessageRFB + "   Java Version " + System.getProperty("java.version") + " ("
    ...
    public static void createPresenter(LectureProfile lectureProfile) {
        try {
            ...
            if (!(lectureProfile.isRecordEnabled() && lectureProfile.isLoopbackRecorder())) {
                ...
                window.getContentPane().addMouseMotionListener(new MouseMotionListener() {
                    ...
                    public void mouseDragged(MouseEvent event) {
                        if (!dragging) {
                            ...
                            point.translate(-position_offset.x, -position_offset.y);
                            ...
                            if (location.x + window.getWidth() > resolution.width)
                                location.x = resolution.width - window.getWidth();
                            if (location.y + window.getHeight() > resolution.height)
                                location.y = resolution.height - window.getHeight();
                ...
                if (lectureProfile.isShowRecordControlsInfo()) {
                    ...
                    JOptionPane.showMessageDialog(window, infoPanel, "TTT Recorder", JOptionPane.INFORMATION_MESSAGE);
            ...
            showMessage("Recorder failed:\n" + "    Server:\t" + lectureProfile.getHost() + " Port: "
    ...
    public static void createOnlinePlayer(InetAddress hostAddress, int port, int accessMode) {
        try {
            // create connection and player
            TTTConnection connection = new TTTConnection(hostAddress, port, accessMode);
            ...
            e.printStackTrace();
            showMessage("Player failed:\n" + "Server: " + hostAddress + " Port: " + port + "\n" + "    Error:\t" + e);
        }
    }

    public static void createScript(File file) {
        try {
            Recording recording = new Recording(file);
            recording.createScript();
            // update thumbnails
            // TODO: ask what to do
            recording.store();
        } catch (Exception e) {
            showMessage("Script generation failed:\n" + (file == null ? "" : "    File:\t" + file + "\n")
                    + "    Error:\t" + e);
        }
    }
    ...
    public void startMessaging() {
    	...
    	if (prot == null) {
    		JOptionPane.showMessageDialog(ttt, "Messaging can only be started, if a teaching session is active!");
    	} else {
    		...
    		while(ip == null && !(result == JOptionPane.CANCEL_OPTION)) {
    			...
    			result = JOptionPane.showConfirmDialog(TTT.this, dlgNI, "select messaging network interface", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
    		...
    		if (result == JOptionPane.OK_OPTION) {
    			...
    			messaging.setLocation(Math.max(0, ttt.desktop.getWidth() - messaging.getWidth()), 100);
    			ttt.desktop.add(messaging);
    			
    			ipFrame = new JInternalFrame("Messaging IP Address");
    			JLabel lblIP = new JLabel(ip.toString().substring(1));
    			lblIP.setFont(((Font)UIManager.get("Label.font")).deriveFont(16f));
    			ipFrame.add(lblIP);
    			ipFrame.pack();
