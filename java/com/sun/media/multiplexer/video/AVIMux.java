// see: http://www.dutchie.net/jmf/bugs/sync.htm
//
//From jmf-interest newsgroup,
//
//I've encountered audio-video synchronization issues
...
public class AVIMux extends com.sun.media.multiplexer.BasicMux {
    ...
    Format littleEndian = new AudioFormat(null, AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED,
            AudioFormat.NOT_SPECIFIED, AudioFormat.LITTLE_ENDIAN, AudioFormat.NOT_SPECIFIED);

    // Little Endian Format for intersecting with the input format.
    Format signed = new AudioFormat(null, AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED,
            AudioFormat.NOT_SPECIFIED, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED);

    Format unsigned = new AudioFormat(null, AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED,
            AudioFormat.NOT_SPECIFIED, AudioFormat.LITTLE_ENDIAN, AudioFormat.UNSIGNED);

    byte[] b1;
    byte[] b2;

    static {
        System.out.println("Revised AVI Multiplexer");
    }
        ...
        if (isVideoFormat) {
            ...
            if (timeStamp - previousTimeStampVideo > avgFrameTime * 1.9) {
                ...
                for (int i = 0; i < blankFrames; i++) {
                    ...
                    blankBuffer.setTimeStamp(previousTimeStampVideo + i * avgFrameTime);
                    ...
                    blankBuffer.setFlags(buffer.getFlags() & ~Buffer.FLAG_KEY_FRAME);
        ...
        if (input instanceof AudioFormat) {
            ...
            if (encoding.equalsIgnoreCase(AudioFormat.LINEAR)) {
                if (af.getSampleSizeInBits() > 8) {
                    ...
                    if (af.getEndian() == AudioFormat.NOT_SPECIFIED || af.getSigned() == AudioFormat.NOT_SPECIFIED) {
            ...
            case WavAudioFormat.WAVE_FORMAT_GSM610:
                if (wavAudioFormat == null) {
                    reason = "A WavAudioFormat is required " + " to provide encoding specific information for this "
                            + "encoding " + wFormatTag;
                    return null;
                }
            }
            ...
            if (frameRate > 0)
                usecPerFrame = Math.round((1 / frameRate) * 1000000);
    ...
    void compareBufs() {
        if (b1.length != b2.length) {
            System.out.println("Bad compare (length " + b1.length + "/" + b2.length + ")");
        } else {
            for (int i = 0; i < b1.length; i++) {
                if (b1[i] != b2[i]) {
                    System.out.println("Bad compare ofs: " + i);
                    break;
                }
            }
        }
    ...
    private Format[] createRGBFormats(Dimension size) {
        ...
        Format[] rgbFormats = new Format[] {
                new RGBFormat(size, size.width * size.height * 2, Format.byteArray, NS, 16, 0x1F << 10, 0x1F << 5,
                        0x1F, 2, size.width * 2, RGBFormat.TRUE, RGBFormat.LITTLE_ENDIAN),
                new RGBFormat(size, size.width * size.height * 3, Format.byteArray, NS, 24, 3, 2, 1, 3, size.width * 3,
                        RGBFormat.TRUE, NS),
                new RGBFormat(size, size.width * size.height * 4, Format.byteArray, NS, 32, 3, 2, 1, 4, size.width * 4,
                        RGBFormat.TRUE, NS) };
        return rgbFormats;
    }
    ...
    private Format[] createYUVFormats(Dimension size) {
        ...
        // UYVY
                new YUVFormat(size, size.width * size.height * 2, Format.byteArray, NS, YUVFormat.YUV_YUYV,
                        size.width * 2, size.width * 2, 1, 0, 2),
                // YUY2
                new YUVFormat(size, size.width * size.height * 2, Format.byteArray, NS, YUVFormat.YUV_YUYV,
                        size.width * 2, size.width * 2, 0, 1, 3),
                // 4:2:0 planar
                new YUVFormat(size, (size.width * size.height * 3) / 2, Format.byteArray, NS, YUVFormat.YUV_420,
                        size.width, size.width / 2, 0, size.width * size.height, (size.width * size.height * 5) / 4),
                // 4:2:0 planar
                new YUVFormat(size, (size.width * size.height * 3) / 2, Format.byteArray, NS, YUVFormat.YUV_420,
                        size.width, size.width / 2, 0, (size.width * size.height * 5) / 4, size.width * size.height) };
        return yuvFormats;
    }
    ...
    private void writeAVIH() {
        ...
        if (totalVideoFrames <= 0) {
            ...
            int computedUsecPerFrame = (int) Math.round(cumulativeInterFrameTimeVideo
                    / (1000. * (totalVideoFrames - 1)));
    ...
    protected void writeFooter() {
        ...
        for (int i = 0; i < numTracks; i++) {
            ...
            if (inputs[i] instanceof VideoFormat) { // Video stream
                int rateVal = 1000000;
                bufClear();
                bufWriteIntLittleEndian(usecPerFrame); // scale
                ...
                if (mp3BitRate > 0) {
                    ...
                    int rate = (int) ((audioFormat.getSampleRate() / factor) * blockAlign);
                    bufClear();
    ...
    private int writeFrame(Buffer buffer, int trackID) {
        ...
        if (length > 0) {
            /*
             * byte[] b = (byte[]) buffer.getData(); int ofs = buffer.getOffset(); if (b1 == null) { b1 = new
             * byte[length]; System.arraycopy(b, ofs, b1, 0, length); } else { b2 = new byte[length];
             * System.arraycopy(b, ofs, b2, 0, length); compareBufs(); b1 = b2 = null; }
             */
        ...
        flag = (buffer.getFlags() & Buffer.FLAG_KEY_FRAME) != 0 ? AVIF_KEYFRAME : 0;
    ...
    protected void writeHeader() {
        ...
        for (int i = 0; i < inputs.length; i++) {
            ...
            if (inputs[i] instanceof AudioFormat) {
                ...
                if (inputs[i] instanceof WavAudioFormat) {
                    ...
                    if ((!af.getEncoding().equalsIgnoreCase(AudioFormat.MPEGLAYER3)) && (codecSpecificHeader != null)
        ...
        for (int i = 0; i < numTracks; i++) {
            ...
            if (isVideo) {
                ...
                if (format instanceof RGBFormat)
                    ...
                    if (yuv.getYuvType() == YUVFormat.YUV_YUYV && yuv.getStrideY() == yuv.getSize().width * 2
                            && yuv.getOffsetY() == 0 && yuv.getOffsetU() == 1 && yuv.getOffsetV() == 3)
                        yuvEncoding = "YUY2";
                    else if (yuv.getYuvType() == YUVFormat.YUV_YUYV && yuv.getStrideY() == yuv.getSize().width * 2
                            && yuv.getOffsetY() == 1 && yuv.getOffsetU() == 0 && yuv.getOffsetV() == 2)
                        yuvEncoding = "UYVY";
                    else if (yuv.getYuvType() == YUVFormat.YUV_YUYV && yuv.getStrideY() == yuv.getSize().width * 2
                            && yuv.getOffsetY() == 0 && yuv.getOffsetU() == 3 && yuv.getOffsetV() == 1)
                        yuvEncoding = "YVYU";
                    else if (yuv.getYuvType() == YUVFormat.YUV_420) {
                        if (yuv.getStrideY() == yuv.getSize().width && yuv.getStrideUV() == yuv.getSize().width / 2) {
            ...
            int strlLength = 0;
            if (isVideo) {
                strlLength = STRH_HEADER_LENGTH + STRF_VIDEO_HEADER_LENGTH + 20 + extraByteLength;
                bufWriteIntLittleEndian(strlLength);
            } else {
                if (extraByteLength > 0) {
                    strlLength = STRH_HEADER_LENGTH + STRF_AUDIO_HEADER_LENGTH + 20 + extraByteLength + 2;
                } else {
                    strlLength = STRH_HEADER_LENGTH + STRF_AUDIO_HEADER_LENGTH + 20;
                }
                bufWriteIntLittleEndian(strlLength);
            }
            ...
            if (isVideo) {
                ...
                if (extraByteLength > 0) {
                    ...
                    bufWriteIntLittleEndian(STRF_AUDIO_HEADER_LENGTH + extraByteLength + 2);
                } else {
                ...
                if (formatTag == null) {
                    ...
                    if (wavAudioFormat != null) {
                        ...
                        if (frameRate != Format.NOT_SPECIFIED) {
                            ...
                            averageBytesPerSecond = (int) audioFormat.getSampleRate() * numChannels
                                    * (sampleSizeInBits / 8);
                        }
        ...
        bufFlush();
        seek(hdrlSizeOffset);
        int hdrlSize = totalStrlLength + AVIH_HEADER_LENGTH + 4 /* bytes */* (1 + // for "hdrl" string
